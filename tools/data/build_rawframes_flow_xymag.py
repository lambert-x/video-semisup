...
def normalize_array(array, out_min=0, out_max=255):
    smin = out_min
    smax = out_max
    array_normalized = (array - array.min()) * (smax - smin) / (array.max() - array.min() + 1e-8) + smin
...
def extract_frame(vid_item):
    ...
    if task == 'rgb':
        if args.use_opencv:
            ...
            os.system(cmd)
    elif task == 'temporal_grad':
        if len(glob.glob(osp.join(full_path, 'tempgrad*'))) > 0:
            return True
        video_name = osp.splitext(osp.basename(vid_path))[0]
        out_full_path = osp.join(out_full_path, video_name)
        if '[' in full_path:
            full_path = full_path.replace('[', '[[]')
        rgb_list = sorted(glob.glob(osp.join(full_path, 'img_*')))
        img_current = None
        img_before = None
        for i, rgb_path in enumerate(rgb_list):
            img_current = mmcv.imread(rgb_path)
            if img_before is None:
                img_before = img_current
                continue
            else:
                temp_grad = img_before.astype(np.float32) - img_current.astype(np.float32)
                temp_grad += 255.0
                temp_grad /= 2.0
                temp_grad = temp_grad.astype(np.uint8)
                img_before = img_current
                mmcv.imwrite(temp_grad, f'{out_full_path}/tempgrad_{i:05d}.jpg')
    elif task == 'flow_xymag':

        video_name = osp.splitext(osp.basename(vid_path))[0]
        out_full_path = osp.join(out_full_path, video_name)
        if '[' in full_path:
            full_path = full_path.replace('[', '[[]')
        flow_x_list = sorted(glob.glob(osp.join(full_path, 'flow_x_*')))
        for i, flow_x_path in enumerate(flow_x_list):
            if os.path.exists(f'{out_full_path}/flow_xym_{i:05d}.jpg'):
                continue
            flow_y_path = flow_x_path.replace('flow_x', 'flow_y')
            flow_x = mmcv.imread(flow_x_path, flag='grayscale').astype(np.float32)
            flow_y = mmcv.imread(flow_y_path, flag='grayscale').astype(np.float32)
            flow_m = np.sqrt(flow_x ** 2 + flow_y ** 2)
            flow_x = normalize_array(flow_x)
            flow_y = normalize_array(flow_y)
            flow_m = normalize_array(flow_m)
            flow_xym = np.dstack([flow_x, flow_y, flow_m])
            flow_xym = flow_xym.astype(np.uint8)
            mmcv.imwrite(flow_xym, f'{out_full_path}/flow_xym_{i:05d}.jpg')
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='flow',
        choices=['rgb', 'flow', 'both', 'temporal_grad', 'flow_xymag'],
