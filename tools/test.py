...
from mmaction.utils.precise_bn import update_bn_stats
...
def precise_bn(model, cfg):
    if cfg.get('precise_bn', False):
        # precise_bn_dataset = build_dataset(cfg.data.train)
        precise_bn_cfg = cfg.get('precise_bn')

        precise_bn_dataset = build_dataset(cfg.data.precise_bn)
        dataloader_setting = dict(
            videos_per_gpu=cfg.data.get('videos_per_gpu_precise_bn', 5),
            workers_per_gpu=2,  # save memory and time
            num_gpus=1,
            dist=True,
            seed=None)
        ...
        if rank == 0:
            print_log(f'Running Precise BN for {precise_bn_cfg.num_iters}  iterations', logger=None)
        update_bn_stats(
            model,
            data_loader_precise_bn,
            precise_bn_cfg.num_iters,
            logger=None,
            bn_range=(precise_bn_cfg.bn_range) if hasattr(precise_bn_cfg, 'bn_range') else None)
        if rank == 0:
            print_log('BN stats updated', logger=None)
