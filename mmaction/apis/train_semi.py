...
def train_model_semi(model,
    ...
    fixmatch_mu = cfg.data.get('fixmatch_mu', None)
    if cfg.omnisource:
    ...
    elif fixmatch_mu is not None:
        dataset_labeled_setting = cp.deepcopy(dataloader_setting)
        dataset_unlabeled_setting = cp.deepcopy(dataloader_setting)
        dataset_unlabeled_setting['videos_per_gpu'] *= fixmatch_mu
        dataloader_settings = [dataset_labeled_setting, dataset_unlabeled_setting]
        data_loaders = [
            build_dataloader(ds, **setting)
            for ds, setting in zip(dataset, dataloader_settings)
        ]
    ...
    Runner = OmniSourceRunner if cfg.omnisource else SemiEpochBasedRunner
    runner = Runner(
        model,
        optimizer=optimizer,
