...
class RawframeDataset_MultiView(RawframeDataset):
    ...
    def __init__(self,
                 ...
                 modality='RGB'):
        assert modality == 'RGB'
        super().__init__(ann_file, pipeline_main, data_prefix, test_mode, filename_tmpl, with_offset,
                         multi_class, num_classes, start_index, modality)
        # self.pipeline_main = Compose(pipeline_main)
        # inherit from base.py:     self.pipeline = Compose(pipeline_main)
        self.pipeline_main = self.pipeline
    ...
    def prepare_train_frames(self, idx):
        ...
        results_main = self.pipeline_main(results)
