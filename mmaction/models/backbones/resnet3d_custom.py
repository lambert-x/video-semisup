...
class ResNet3d_Custom(nn.Module):
    ...
    def __init__(self,
        ...
        self.feature_diff_indices = feature_diff_indices
    ...
    def forward(self, x):
        ...
        if -2 in self.feature_diff_indices:
            x = x[:, :, 1:, ...] - x[:, :, :-1, ...]

        x = self.conv1(x)

        if -1 in self.feature_diff_indices:
            x = x[:, :, 1:, ...] - x[:, :, :-1, ...]
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            x = res_layer(x)
            if i in self.feature_diff_indices:
                x = x[:, :, 1:, ...] - x[:, :, :-1, ...]
                # print(x.shape)
