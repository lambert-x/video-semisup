...
class SemiAppTempBaseRecognizer(BaseRecognizer):
    ...
    def __init__(self, backbone, cls_head,
                 cls_head_temp, temp_contrast_head, temp_backbone='same', temp_sup_head='same',
        ...
        if self.temp_align_indices is not None:
            ...
            backbone.out_indices = self.temp_align_indices
            temp_backbone.out_indices = self.temp_align_indices
    ...
    def forward_train(self, imgs, labels, imgs_weak, imgs_strong, labels_unlabeled, imgs_appearance,
                      imgs_diff_labeled, imgs_diff_unlabeled):
    ...
    def forward(self, imgs, label=None, return_loss=True, imgs_weak=None, imgs_strong=None, label_unlabeled=None,
                imgs_appearance=None, imgs_diff_labeled=None, imgs_diff_unlabeled=None, **kwargs):
        ...
        if return_loss:
            ...
            return self.forward_train(imgs, label, imgs_weak, imgs_strong, label_unlabeled, imgs_appearance,
                                      imgs_diff_labeled, imgs_diff_unlabeled, **kwargs)
        else:
    ...
    def train_step(self, data_batch_labeled, data_batch_unlabeled, optimizer, **kwargs):
        ...
        imgs_diff_unlabeled = data_batch_unlabeled['imgs_diff']
        if self.app_backbone is None:
            imgs_appearance = None
        else:
            imgs_appearance = torch.cat([data_batch_labeled['imgs_appearance'], data_batch_unlabeled['imgs_appearance']], dim=0)
        ...
        losses = self(imgs, label, imgs_weak=imgs_weak, imgs_strong=imgs_strong, label_unlabeled=label_unlabeled,
                      ...
                      imgs_diff_unlabeled=imgs_diff_unlabeled, **kwargs)
        # print(losses)
