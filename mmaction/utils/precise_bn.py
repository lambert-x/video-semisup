...
@torch.no_grad()
def update_bn_stats(model, data_loader, num_iters=200, logger=None, bn_range=None):
    ...
    if bn_range is None:
        ...
        for attr in bn_range:
            selected_modules += list(getattr(model, attr).modules())
            bn_layers = [
                m for m in selected_modules if m.training and isinstance(m, _BatchNorm)
            ]
...
class PreciseBNHook(Hook):
    ...
    def __init__(self, dataloader, num_iters=200, interval=1, bn_range=None):
